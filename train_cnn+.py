...
def batch_data_gen(input_size, list_name0, list_name1, file_name, features, index_file_path, data_path, batch_size=16):
    while True:
        ...
        for batch_index, label_list in index_gen:
            data_block = []
            input_feature = []
            key_names = features.keys()
            for index in batch_index:
                feature_name = [x for x in key_names if x in index]
                input_feature.append(features[feature_name[0]])
                data = np.load(index)
            ...
            #     plt.show()
            input_feature = np.asarray(input_feature).astype(np.float32)
            yield ([input_data, input_feature], input_label)


def trainModel(input_size1, input_size2, EPOCHS, Batch_size, STEPS_PER_EPOCH, val_steps):
    trainGenerator = batch_data_gen(input_size1, list_name0, list_name1, 'train_index.npy', features, index_file_path, data_path, batch_size=Batch_size)
    validationGenerator= batch_data_gen(input_size1, list_name0, list_name1, 'val_index.npy', features, index_file_path, data_path, batch_size=Batch_size)
    callbacks = [
        TensorBoard(),
        ...
        ModelCheckpoint(filepath='RESULT2/valloss{val_loss:.3f}_loss{loss:.3f}_classify_epoch{epoch:03d}.hdf5',
                        monitor='val_loss',
                        mode='min', period=1)
    ]
    model = CNN_radiologist(input_size1, input_size2)
...
if __name__ == '__main__':
    ...
    os.makedirs('RESULT')
    trainModel(input_size1, input_size2, EPOCHS, BATCH_SIZE, STEPS_PER_EPOCH=21, val_steps=5)
